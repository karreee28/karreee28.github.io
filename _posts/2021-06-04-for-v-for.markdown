...
- 구조 
큰 리스트를 for문으로 돌리고, 각각이 가진 리스트를 for문으로 또 돌림. 
큰 리스트 클릭 시, 각각의 리스트가 펼쳐짐 (그래서 각 데이터 배열에 isOpen이라는 boolean값을 둠)
각각의 리스트에는 리스트에 추가할 수 있는 항목이 있음(이건 for문 안돌림 - 방식은 동일)dd

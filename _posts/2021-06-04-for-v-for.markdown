...
<div class="edit" v-for="pet in pets" :key="pet.pet_id">
  <!-- 배열을 for문으로 돌림 -->
  <el-button size="small" class="submit-btn" @click="postPet">저장</el-button>
...
<div class="add" v-for="(pet, index) in newPets" :key="index">
  ...
  <el-button size="small" class="submit-btn" @click="putPet(pet)">저장</el-button>
  <el-row class="form-row">
    <el-col class="form-col">
      <div class="form-label">구분</div>
      <div class="form-cont">
...
```
저장버튼(postPet)을 누르면 입력 되어있는 데이터를 API로 저장 <br>
...
```javascript
methods: {
  postPet: async function() {
    // API로 데이터 post (보통 기존 데이터 수정 시 사용)
  },
  putPet: async function(item) {
    // API로 데이터 put (데이터 추가 시 사용)
  },
  getList: async function() {
    // 보통 API로 데이터를 불러옴. 아래 배열은 임시 데이터. 
    // 데이터 수정일 경우 사용
    let rs = [
      {
        pet_id: 1,
